...
public class UserService_updateUserTest
{
    @InjectMocks
    ...
    @Mock
    private HttpEntity<UpdateQueryRQ> httpEntity;
    @Mock
    ...
    private void setupCommonService()
    {
        ...
        when(commonService.getHttpEntityForUpdate(any(UpdateQueryRQ.class))).thenReturn(httpEntity);
    }

    private void setupRestTemplate()
    {
        when(apiClientUtils.getRestTemplate()).thenReturn(restTemplate);
        when(restTemplate.postForEntity("/update", httpEntity, ServiceResult.class)).thenReturn(responseEntity);
    ...
    {
        //given
        UpdateUserRQ updateUserRQ = new UpdateUserRQ("ABCD", "name", "email@email.com", "USER");

        // when
        userService.updateUser(updateUserRQ);
    ...
    public void shouldPostForEntityUsingRestTemplate() throws JSONException
    {
        ...
        // then
        verify(restTemplate).postForEntity("/update", httpEntity, ServiceResult.class);
    }
}
